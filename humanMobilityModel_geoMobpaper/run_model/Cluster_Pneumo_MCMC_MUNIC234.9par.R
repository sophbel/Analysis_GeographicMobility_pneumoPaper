...
#### Bring in data
if(cluster==TRUE){
setwd("/lustre/scratch118/infgen/team284/sb62/Migration/SouthAfrica/mobility_model/SA_Migration_130422/RData_outputs/")}else{
...
load("pairwise_geodist.town.RData") 
...
source("/lustre/scratch118/infgen/team284/sb62/Migration/SouthAfrica/mobility_model/SA_Migration_130422/MCMC_model/LikelihoodFunctions/LikFunc.mcmc.Munic.234.9par.R")}else{
sourceCpp("/Users/sb62/Documents/Migration/SA_Migration_110422/MCMC_model/MatrixMultiplication.cpp")
source("/Users/sb62/Documents/Migration/SA_Migration_110422/MCMC_model/LikelihoodFunctions/LikFunc.mcmc.Munic.234.9par.R")}
...
start.time<-Sys.time()
ans.munic234 <- MCMC(likFunc.munic.234,initial = startPar,nsteps  = iters,kernel  = kernel_normal(scale = .08),thin=5)
end.time<-Sys.time()
end.time-start.time
if(cluster==TRUE){
setwd("/lustre/scratch118/infgen/team284/sb62/Migration/SouthAfrica/mobility_model/SA_Migration_130422/MCMC_model/RunModel/outputs/munic234/")
save(ans.munic234,file=paste0("ans.munic234.9par.",chain,".",iters,".08",".RData"))}
# }
...
#####
# setwd("/Users/sb62/Documents/Migration/SA_Migration_110422/MCMC_model/RunModel/outputs/munic234/")
# load("ans.munic2341.4000.08.RData")
# chain1<-ans.munic234
# chain1<-mcmc(chain1,start=200,end=800)
# infec.probpars<-summary(chain1)$statistics[2:9,1]
# 
# # testpars<-seq(-5,-1,1)
# testpars<-seq(-3,-1,0.5)
# 
# ll<-rep(NA,length(testpars))
# for(par in 1:length(testpars)){
#   pars.test<-c(testpars[par],infec.probpars)
#   # ll[par]<-likFunc.munic.234(pars.test)
#   ll[par]<-likFunc.munic.234(testpars[par])
#   
#   print(paste0(par,"/",length(testpars)))
# }
# plot(testpars,ll)
