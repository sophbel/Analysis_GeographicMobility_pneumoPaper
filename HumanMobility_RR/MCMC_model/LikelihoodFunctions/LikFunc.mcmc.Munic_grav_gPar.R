likFunc.munic<-function(par){
  ...
  extTranMatDat.tmp$pars$homeSus <- par[2]
  ...
  probMove_preadjtmp <- apply(probMove, 2, function(x) x/rowtot  )
  ...
  probMove_preadj<-sweep(tmpbase_pre1,1,(1-tmpdiag)/(1-diag(tmpbase_pre1)),"*")
  diag(probMove_preadj)<-tmpdiag
