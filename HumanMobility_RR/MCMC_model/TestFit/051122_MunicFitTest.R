...
### all chains
load(paste0("./MCMC_model/outputs/ans.munic1.",iters,".08.RData"))
chain1<-ans.munic
chain1<-chain1[1000:4000,]
load(paste0("./MCMC_model/outputs/ans.munic2.",iters,".08.RData"))
chain2<-ans.munic
chain2<-chain2[1000:4000,]
load(paste0("./MCMC_model/outputs/ans.munic3.",iters,".08.RData"))
chain3<-ans.munic
...
################################################################################################
#######TRUE DATA
## Bootstrap the blue line (ie data)
...
cummeanDist.emp.bs<-meanDist.emp.bs<-matrix(NaN,length(gDistMax),nboot)

for(j in 1:nboot){
  sampids<-sample(un.ids,replace=T)
  samp<-which(id.1%in%sampids&id.2%in%sampids)
  for (i in 1:length(gDistMax)){
    a<-which(dat.inMaster$totEvoTime[samp]>=gDistMin[i]&dat.inMaster$totEvoTime[samp]<gDistMax[i])
    b<-which(dat.inMaster$totEvoTime[samp]<gDistMax[i])

    meanDist.emp.bs[i,j]<-mean(dat.inMaster$SDist[samp[a]])
    cummeanDist.emp.bs[i,j]<-mean(dat.inMaster$SDist[samp[b]])
  }
  print(j)
}
# meanDist.emp.bs[i,]
...
xpoly<-c(gDistMid,rev(gDistMid))
ypoly1<-c(meanDist.emp.bs.qt[1,],rev(meanDist.emp.bs.qt[3,]))
ypolycum<-c(cummeanDist.emp.bs.qt[1,],rev(cummeanDist.emp.bs.qt[3,]))
