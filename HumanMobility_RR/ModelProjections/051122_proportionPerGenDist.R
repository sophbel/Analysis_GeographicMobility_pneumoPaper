...
load("ans.munic1.20000.08.RData")
chain1<-ans.munic
chain1<-chain1[1000:4000,]
load("ans.munic2.20000.08.RData")
chain2<-ans.munic
chain2<-chain2[1000:4000,]
load("ans.munic3.20000.08.RData")
chain3<-ans.munic
...
dist.prop.mat.list <-dist.propSingle.list<-list()
...
prop.mat <-dist.propSingle<-matrix(nrow=locs,ncol=length(distmax))
for (j in 1:length(genvec)){
  for (n in 1:locs){
    # startSamp=n
    # startSamp<-sample(234,1,replace=T,prob=pop2019.town)
...
### Find quantiles across bootstraps individual person
...
# dist.prop.mat2.tmp$gens <- rep(as.character(genvec),iters)
dist.propSingle.tmp$gens <- "Individual"
...
dist.prop.mat2<-rbind(dist.prop.mat2.tmp,dist.prop.mat.rand.tmp,dist.propFB,dist.propSingle.tmp)
dist.prop.mat2 <- melt(dist.prop.mat2)
colnames(dist.prop.mat2) <- c("gens","distances","value")
dist.prop.mat2$distances <- factor(dist.prop.mat2$distances,levels=c("0-10km","10-200km","200-500km","500-1000km",">1000km"),labels=c("Within Municipality" ,"10-200km","200-500km","500-1000km",">1000km"))
genvec2<-c(1,10,"Individual","Random","Meta")
...
# dist.quants.tmp3 <- subset(dist.quants, dist.quants$gens==1 | dist.quants$gens==10 | dist.quants$gens=="Random")
dist.quants.tmp3 <- subset(dist.quants, dist.quants$gens==1 | dist.quants$gens==10 | dist.quants$gens=="Random"
                           | dist.quants$gens=="Meta" | dist.quants$gens=="Individual")
...
setwd("/Users/sb62/Documents/Migration/SA_Migration_110422/ModelProjections/proppergen/")
...
ggplot(data=dist.quants.tmp3) +
  geom_point(data=dist.quants.tmp3,aes(x=distances,y=median,shape=gens),size=3,position=position_dodge(width=0.35)) +
  geom_errorbar(aes(ymin=lowerCI,ymax=upperCI,x=distances,shape=gens),alpha=0.6,width=0.5,position=position_dodge(width=0.35))+
  xlab("Distance \n(from starting municipality)")+
  ylab("Proportion")+
  theme(panel.grid.minor  = element_blank(),)+
  theme_classic()+
